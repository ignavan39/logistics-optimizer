...
### ✅ Makefile + OSRM для демо

**Создано**:
1. **Makefile** — стандартные команды:
   - `make install` — pnpm install
   - `make osrm` — скачать OSM данные России (~12GB)
   - `make init` — install + osrm + docker pull
   - `make build` — pnpm build
   - `make up` — Docker (docker compose -f ....yml -f ...services.yml up -d)
   - `make up:dev` — Docker + фронтенд
   - `make web` — только фронтенд
   - `make down` / `make restart` / `make logs`
   - `make test` / `make lint` / `make typecheck`
   - `make clean` / `make clean:all`
2. **.gitignore** — добавлен `osrm-data/`

### ✅ README + Screenshots

**Обновлено README.md**:
- Key Features: OSRM приоритет (первая строка)
- Screenshots: 4 изображения (dashboard, tracking, counterparts, settings)
- URLs: добавлены OSRM :5000, MinIO :9000
- Быстрый старт: make команды

**Скриншоты в assets/demo/**:
- `counterparties.png` — страница контрагентов
- `settings.png` — настройки с вкладками (пользователи, аудит)

### ✅ Тестовые ИНН

**Проблема**: Реальные ИНН в БД = юридический риск для демо

**Решение**: Заменить на тестовые

**Где**:
| Таблица | Было | Стало |
|---------|-----|-------|
| counterparty.inn | 1234567890 | 1234567890 |
| counterparty.inn | 1234567891 | 1234567891 |
| counterparty.inn | 1234567892 | 1234567892 |
| counterparty.inn | 1234567893 | 1234567893 |
| counterparty.inn | 1234567894 | 1234567894 |
| order_db.settings.company_inn | 1234567890 | 1234567890 |

**Команды**:
```bash
# Просмотр
docker exec logistics-pg-counterparty psql -U logistics -d counterparty_db -c "SELECT name, inn FROM counterparty;"

# Обновление
docker exec logistics-pg-counterparty psql -U logistics -d counterparty_db -c "UPDATE counterparty SET inn = '1234567890' WHERE name LIKE '%Тест%';"
```

**Сид-файл**: `infra/postgres/seeds/02-counterparty.sql` — обновлены ИНН

### ✅ docs/SERVICES.md — секция OSRM

Добавлена секция "OSRM (Open Source Routing Machine)" после routing-service:
- Docker: osrm container
- Endpoints: route, table
- Data: osrm-data/russia-latest.osrm (~12GB)
- Download: `make osrm`

### ✅ docs/COMMUNICATION.md — External Services

Добавлена секция "External Services":
| Service | Docker | URL | Purpose |
|---------|--------|-----|---------|
| OSRM | osrm | http://osrm:5000 | Route calculation |
| MinIO | minio | http://minio:9000 | S3 storage |
| Prometheus | prometheus | http://prometheus:9090 | Metrics |
| Grafana | grafana | http://grafana:3001 | Dashboards |
| Jaeger | jaeger | http://jaeger:16686 | Tracing |

## Выводы

### Паттерны

1. **Makefile = стандартизация** — одна команда вместо длинных docker/pnpm команд
2. **Тестовые данные** — все ИНН, телефоны, адреса = тестовые (1234567890+)
3. **Докумен��ируй сразу** — нашёл OSRM только через grep docker-compose
4. **Скриншоты демо** — assets/demo/

### Что трудно

1. **Много файлов** — 37 изменённых файлов = сложно держать в голове
2. **Read-only режим** — ждать подтверждения замедляет
3. **Где данные в БД** — искать по docs/DATABASE.md

### Совет на будущее

1. **Стандартные команды** — ВСЕГДА через Makefile
